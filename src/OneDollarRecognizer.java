...
public class OneDollarRecognizer {
	...
	public void recognize() {
		...
		for(Template T : theTemplates){
			...
			if(d<b){
				b = d;			
				Template T1 = new Template(T.getName(), T.getPoints());
	...
	protected Vector<Tuple2> rotateToZero(Vector<Tuple2> points) {
		...
		Tuple2 c = centroid(points);//computes vector of (x,y)
		double theta = Math.atan((c.y-points.get(0).y)/(c.x - points.get(0).x));
	...
	protected Vector<Tuple2> rotateBy(Vector<Tuple2> points, double theta)
	{
		...
		for (Tuple2 p : points) { 
			Tuple2 q= new Tuple2();
				q.x = (p.x - c.x) * Math.cos(theta) - (p.y - c.y) * Math.sin(theta) + c.x;
				q.y = (p.x - c.x) * Math.sin(theta) - (p.y - c.y) * Math.cos(theta) + c.y;
				newPoints.add(q);
			}   
		return newPoints;
	}	
